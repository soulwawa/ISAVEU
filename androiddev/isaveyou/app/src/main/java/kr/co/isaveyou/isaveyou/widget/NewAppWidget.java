...
import kr.co.isaveyou.isaveyou.notification.MyFirebaseMessagingService;
...
//        List<String> keys = new ArrayList<>();
        ...
        for (Map.Entry<String, ?> entry : allEntries.entrySet()) {
            ...
            switch (entry.getKey()) {
                ...
                case "3":
                    ...
                    Log.v(TAG, "화재/지진");
                    sharedPreferences.edit().remove("3").apply();
                    break;
                case "4":
    ...
    class WidgetUpdateTask extends AsyncTask {
        ...
        protected Object doInBackground(Object[] objects) {
            ...
            try {
                SharedPreferences sharedPreferences1 = context.getSharedPreferences("check_type", Context.MODE_PRIVATE);
                ...
                for (Map.Entry<String, ?> entry : allEntries.entrySet()) {
                    switch (entry.getKey()) {
                        case "1":
                            ...
                            Log.v(TAG, "화재");
                            sharedPreferences1.edit().remove("1").apply();
                            break;
                        case "2":
                            issue2 = sharedPreferences1.getString("2", "");
                            Log.v(TAG, "지진");
                            sharedPreferences1.edit().remove("2").apply();
                            break;
                        case "3":
                            issue2 = sharedPreferences1.getString("3", "");
                            Log.v(TAG, "화재/지진");
                            sharedPreferences1.edit().remove("3").apply();
                            break;
                        case "4":
                            issue2 = sharedPreferences1.getString("4", "");
                            Log.v(TAG, "소화기");
                            sharedPreferences1.edit().remove("4").apply();
                            break;
                        case "5":
                            issue2 = sharedPreferences1.getString("5", "");
                            Log.v(TAG, "기계이상");
                            sharedPreferences1.edit().remove("5").apply();
                            break;
                        default:
                            issue2 = "";
                            break;
