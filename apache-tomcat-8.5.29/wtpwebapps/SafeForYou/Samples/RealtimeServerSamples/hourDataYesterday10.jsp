...
// requestAllData = true 인 경우 어제의 현재 시(hour):00 부터 어제의 현재 시 + 1까지 10초 간격 데이터를 생성합니다.
// requestAllData = false 인 경우 어제의 현재 시:분에 해당하는 데이터 하나를 생성하게 됩니다.
// 차트 레이아웃에서 설정하는 interval 속성은 본 예제에서 사용하지 않습니다.
// interval 을 설정하지 않았다는 것은 주기 단위로 기존 데이터에 adding 을 하지 않는 다는 의미입니다.
...
//****************************************************************************************************************************
	...
	// 어제의 현재 시간(분절삭) = 현재 시간에서 분(minutes)를 제거한 00분을 시작 시간으로 설정 - 하루.
	...
	// <startDate> 노드 없으므로 디폴트로 생성된 데이터의 초기 데이터값 참조하여 생성.
	...
	for(int i=0; i<=gap; i+=1000*10) //10초단위 데이터.
	{
		// 랜덤 데이터 생성.
